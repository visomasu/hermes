# Hermes Production Launch Script (Local)
# Starts Hermes in Production mode on port 3979
...
Write-Host "=== Hermes Production Instance (Local) ===" -ForegroundColor Cyan
Write-Host ""
...
Write-Host "  Backend Port: 3979" -ForegroundColor White
Write-Host "  Backend URL: http://localhost:3979" -ForegroundColor White
Write-Host ""
...
if (-not $NoPlayground) {
    ...
    if ($playgroundInstalled) {
        ...
        $playgroundCommand = "agentsplayground -e `"http://localhost:3979/api/messages`" -c `"emulator`""
...
try {
    ...
    if ($NoBuild) {
        dotnet run --no-build --configuration Release --urls "http://localhost:3979" --environment Production
    }
    else {
        dotnet run --configuration Release --urls "http://localhost:3979" --environment Production
    }
}
finally {
    Pop-Location
